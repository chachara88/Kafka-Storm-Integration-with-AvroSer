...
public class AvroKafkaSpoutConfig<K, V> extends CommonKafkaSpoutConfig<K, V> {
    ...
    public static AvroKafkaSpoutConfig.Builder<String, String> builder(String bootstrapServers, String... topics) {
        return (new AvroKafkaSpoutConfig.Builder(bootstrapServers, topics)).withCustomAvroDeserializers();
    }
    ...
    public static class Builder<K, V> extends org.apache.storm.kafka.spout.internal.CommonKafkaSpoutConfig.Builder<K, V, AvroKafkaSpoutConfig.Builder<K, V>> {
        ...
        public Builder(String bootstrapServers, String... topics) {
            ...
            for (String topicName: topics) {
                if (topicName.equals("aeroloop_SensorReadingScalar")) {
                    LOG.warn("ApacheStormMachine -->" + " TopicName is: " + topicName);
                    this.isSpecificTopic = true;
                    LOG.warn("ApacheStormMachine -->" + " isSpecificTopic var is set up to true");
                    break;
                }
            }
        }
        ...
        public AvroKafkaSpoutConfig.Builder<K, V> setProcessingGuarantee(
                AvroKafkaSpoutConfig.ProcessingGuarantee processingGuarantee) {
        ...
        private AvroKafkaSpoutConfig.Builder<K, V> withStringDeserializers() {
            this.setProp("key.deserializer", /*LongDeserializer*/StringDeserializer.class.getName());
            this.setProp("value.deserializer", StringDeserializer.class.getName());//<----------------------
            return this;
        }
        ...
        private AvroKafkaSpoutConfig.Builder<K, V> withCustomAvroDeserializers() {
            ...
            this.setProp(KafkaAvroDeserializerConfig.SCHEMA_REGISTRY_URL_CONFIG,
                    "http://IP_ADDRESS:PORT_NUM"); //<--- Run Schema Registry on PORT_NUM
            return this;
        }
        ...
        private AvroKafkaSpoutConfig.Builder<K, V> setKafkaPropsForProcessingGuarantee() {
            if (this.getKafkaProps().containsKey("enable.auto.commit")) {
                throw new IllegalStateException(
                        "The KafkaConsumer enable.auto.commit setting is not supported. You can configure similar " +
                ...
                if (this.processingGuarantee == AvroKafkaSpoutConfig.ProcessingGuarantee.AT_LEAST_ONCE) {
                    if (autoOffsetResetPolicy == null) {
                        ...
                        AvroKafkaSpoutConfig.LOG.warn("" +
                                "Cannot guarantee at-least-once processing with auto.offset.reset.policy other than " +
                                "'earliest' or 'none'. Some messages may be skipped.");
                    }
                } else if (this.processingGuarantee == AvroKafkaSpoutConfig.ProcessingGuarantee.AT_MOST_ONCE &&
                        autoOffsetResetPolicy != null && !autoOffsetResetPolicy.equals("latest") &&
                        !autoOffsetResetPolicy.equals("none")) {
                    AvroKafkaSpoutConfig.LOG.warn("Cannot guarantee at-most-once processing with " +
                            "auto.offset.reset.policy other than 'latest' or 'none'. " +
                            "Some messages may be processed more than once.");
                }

                AvroKafkaSpoutConfig.LOG.info("Setting Kafka consumer property '{}' to 'false', " +
                        "because the spout does not support auto-commit", "enable.auto.commit");
    ...
    public static enum ProcessingGuarantee {
        AT_LEAST_ONCE, AT_MOST_ONCE, NO_GUARANTEE;
